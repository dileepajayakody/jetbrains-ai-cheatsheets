...
function buildPrompt(product: ProductConfig, markdown: string, seed: string): string {
  return [
    ...
    '- 12–22 color-coded sections, each 5–12 rows. Distribute sections evenly across the 4 columns (roughly balanced).',
    ...
    '- Vary `colorClass` (s1–s12) and `icon` (a single emoji) across sections for visual rhythm.',
    ...
    '- Keep it practical and demo-oriented: how to start, key surfaces, commands, controls, pro tips. Do not invent features not present in the docs.',
    '',
    '=== EDITORIAL SEED ===',
    seed || '(none provided)',
    '',
