...
1.  Open the finished task and click Changes to review the diff.

    Iterate on feedback when needed.

    Learn more in [Review changes](review-changes.html).

2.  Use agentic review when appropriate.

    You can also ask an agent to review the diff. Agentic review is useful for structured checks, but you should still verify critical behavior manually.
