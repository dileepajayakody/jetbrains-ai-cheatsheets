...
2.  [Model Context Protocol (MCP)](#0)
...
Import MCP servers from the Claude config file.
...
Click ! in the Level column to specify whether the added MCP server must be available globally or only in the current project.

Automatically enable new and changed MCP servers

Enable this setting to automatically start newly added MCP servers or servers whose configuration was changed.

For additional information, refer to [Model Context Protocol (MCP)](mcp.html).
